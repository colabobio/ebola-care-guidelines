...
public class LogRegModel {
  ...
  void addTerm(ModelTerm t) {
    terms.put(t.varName, t);
  }
  ...
  public float eval(HashMap<String, Float> values) {
    ...
    return (float)sigmoid.value(score);
  }
  ...
  void loadTerms(InputStream is) {
    ...
    while(true) {
      ...
      String var = s.substring(0, pos0).trim();

      if (var.indexOf("rcs") == 0) {
        int pos1 = var.lastIndexOf(")");
        String rcsString = var.substring(4, pos1);
        ...
        int coeffOrder = var.length() - var.replace("'", "").length();
        if (coeffOrder == 0) {
        ...
        if (coeffOrder == rcsOrder - 2) {
          ModelTerm term = new RCSTerm(varName, rcsOrder, rcsCoeffs, rcsKnots);
          addTerm(term);
        }
      } else {
        if (var.equals("(Intercept)")) {
          ...
          ModelTerm term = new LinearTerm(var, value);
          addTerm(term);
        }
      }
    }
