...
public class PredSketch extends PApplet {
  ...
  String[] varSymptom = { "High viral load", "Age", "Fever",
                          "Jaundice", "Bleeding", "Headache",
                          "Diarrhoea", "Vomiting",
                          "Abdomen pain", "Weakness",
                          "Wellness" };
  ...
  private void scaleDimensions() {
    ...
    titleFontSize *= density;
    labelFontSize *= density;
    valueFontSize *= density;
...
//    navHeight = 0;
    ...
    density = surface.getActivity().getResources().getDisplayMetrics().density;
    scaleDimensions();
...
//      }
  ...
  private void drawDetPred() {
    ...
    line(riskX, yTop + 2 * density, riskX, yTop + riskHeight - 2 * density);
  ...
  private void startTreatActivity(int i) {
    String var = varOrder[i];
    Intent intent = new Intent(surface.getActivity(), TreatActivity.class);
